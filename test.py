...
name_dir_pcd is the name of folder containing persistence diagrams as txt files
...
If the number of birth-death generator is large, I recommend to use approx=True
because it may take several days or months.
"""
...
def function_weight(_name_weight, _val_c=1.0, _val_p=1):
    if _name_weight == "arctan":
        def _func_weight(vec_bd):
            return np.arctan(np.power((vec_bd[1] - vec_bd[0]) / _val_c, _val_p))
    else:  # p-persistence
        def _func_weight(vec_bd):
...
"""
tda.plot_gram(mat_gram, show=True)


"""
You can also see the squared distance matrix
mat_distance[i, j] = || E_{k}(¥mu^{w}_{D}), E_{k}(¥mu^{w}_{E})||_{¥cH_{k}}^{2}
"""
mat_distance = class_pwk.mat_distance
tda.plot_gram(mat_distance, show=True)
